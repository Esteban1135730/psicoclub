"use client";

const WHATSAPP_NUMBER = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER ?? "573001234567";

/** Mismo estilo pegatina 3D que los iconos del footer */
const STICKER_SHADOW_3D =
  "shadow-[inset_2px_2px_6px_rgba(0,0,0,0.25),inset_-2px_-2px_4px_rgba(255,255,255,0.2),3px_3px_8px_rgba(0,0,0,0.25)]";
const STICKER_HOVER_SHADOW =
  "hover:shadow-[inset_2px_2px_6px_rgba(0,0,0,0.25),inset_-2px_-2px_4px_rgba(255,255,255,0.2),4px_4px_12px_rgba(0,0,0,0.28)]";
const ICON_OUTLINE_STYLE: React.CSSProperties = {
  filter:
    "drop-shadow(0 1px 0 #1e293b) drop-shadow(0 -1px 0 #1e293b) drop-shadow(1px 0 0 #1e293b) drop-shadow(-1px 0 0 #1e293b) drop-shadow(1px 1px 0 #1e293b) drop-shadow(-1px -1px 0 #1e293b) drop-shadow(1px -1px 0 #1e293b) drop-shadow(-1px 1px 0 #1e293b)"
};

export function WhatsAppButton() {
  const url = `https://wa.me/${WHATSAPP_NUMBER.replace(/\D/g, "")}`;

  return (
    <a
      href={url}
      target="_blank"
      rel="noopener noreferrer"
      className={`fixed bottom-6 right-6 z-50 flex items-center justify-center rounded-full border-2 border-slate-800 text-white transition duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-slate-700 focus:ring-offset-2 ${STICKER_SHADOW_3D} ${STICKER_HOVER_SHADOW} h-16 w-16 sm:h-[4.5rem] sm:w-[4.5rem]`}
      style={{
        background: "linear-gradient(135deg, #6ee7b7 0%, #6ee7b7 68%, rgba(255,255,255,0.25) 100%)"
      }}
      aria-label="Abrir WhatsApp"
    >
      <span className="inline-flex shrink-0 rotate-[-2deg]" style={ICON_OUTLINE_STYLE}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="currentColor"
          className="h-8 w-8 sm:h-9 sm:w-9"
          aria-hidden
        >
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
        </svg>
      </span>
    </a>
  );
}
